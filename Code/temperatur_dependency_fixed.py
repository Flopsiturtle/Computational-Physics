...
for B in ['0.000500' , '0.001000', '0.005000',  '0.010000']:
    ...
    beta_list = []
    names = []
    for name in glob.glob('C:\\Users\\Mickey\\Desktop\\Computational-Physics-1\\Results2\\EnB' + str(B) +'Beta*.csv'):
        names.append(name)
...
for B in ['0.000500' , '0.001000', '0.005000',  '0.010000']:
    ...
    beta_list = []
    names = []
    for name in glob.glob('C:\\Users\\Mickey\\Desktop\\Computational-Physics-1\\Results4\\MagB' + str(B) +'Beta*.csv'):
        names.append(name)
        beta_list.append(float(name[-12:-4]))   #watch out this needs to work with the given beta values
        column_names = ['replica_number' + str(i+1) for i in range(R+1)]
        energydata =  pd.read_csv(name, names = column_names, index_col=False) 

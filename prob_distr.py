...
######### maybe own distribution of errors (easy implementation) - but then no meaning of which values have which errors
...
''' --- my own version --- '''
# ----- does the same except you have to choose an error for max(array) which can lead to wrong results -> if statement in function...
...
def prob_distr(probs_array,num_bars):
    ...
    for i in np.arange(num_bars):
        number = probs[(probs>=(min+bar_size*i)) & (probs<(max+10**(-10)+bar_size*(i+1-num_bars)))]

...
epsilon = 1/60
"""
data = get_data(4,200,400,40,mu,epsilon)     ### maybe delete input with start,finish,gap to be always the same and give input of errors and maxiters, depending on usecase we want for final test
...
def plot_parity():
    v = np.concatenate((np.random.random(100),[0])) 
    ...
    plot_eigenvectors(np.concatenate((Eig_val_even,Eig_val_odd)),np.concatenate((Eig_vec_even,Eig_vec_odd)))

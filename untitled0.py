...
def test_hermiticity(Hamilton_operator, psi_in, iterations, error):
    ...
    psi1 = np.random.rand(*shape) + 1j * np.random.rand(*shape)
    psi2 = np.random.rand(*shape) + 1j * np.random.rand(*shape)
    LHS = np.sum(np.multiply(psi1, Hamiltonian(psi2)))
    RHS = np.sum(np.multiply(Hamiltonian(psi1), psi2))
    if (np.abs(LHS - RHS) < error): 
        print('Hermitian!')
    else: 
        print('Alarm, Hamiltonian not hermitian')
        
        
def test_positivity(Hamilton_operator, potential_operator, psi_in, iterations, error):
    shape = np.shape(psi_in)
    psi1 = np.random.rand(*shape) + 1j * np.random.rand(*shape)
    if (np.any(potential_operator(psi1)) < 0):
        print('Potential is not positive!')
    elif np.any(np.multiply(psi1, Hamiltonian(psi1)) < 0):
        print(np.multiply(psi1, Hamiltonian(psi1)))
        print('Alarm, Hamiltonian not positive')
    else:
        print('Hamiltonian is positive')
    
    
            


test_linearity(Hamiltonian, initial, 1, 0.001)
test_hermiticity(Hamiltonian, initial, 1, 0.001)
test_positivity(Hamiltonian,Potential, initial, 1, 0.001)
